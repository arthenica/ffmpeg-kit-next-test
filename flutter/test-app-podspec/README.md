# FFmpegKitNext Flutter Test Application (CocoaPods)

This Flutter test application consumes the local `ffmpeg_kit_next_flutter` plugin. On Apple platforms it uses the
plugin podspec and CocoaPods.
...
Keep this in `android/build.gradle`, not in `settings.gradle` dependency resolution. For iOS, iPadOS and macOS,
Flutter's normal CocoaPods integration uses the plugin podspec, which vendors the copied `xcframeworks`; do not add an
extra `pod` declaration for FFmpegKitNext.
