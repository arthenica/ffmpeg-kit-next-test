...
type ConcurrentExecutionTabProps = TabProps<'CONCURRENT EXECUTION'>;

interface ConcurrentExecutionTabState {
    outputText: string;
    sessionId1: number;
    sessionId2: number;
    sessionId3: number;
...
export default class ConcurrentExecutionTab extends React.Component<ConcurrentExecutionTabProps, ConcurrentExecutionTabState> {

    private scrollViewReference: ScrollView | null = null;

    constructor(props: ConcurrentExecutionTabProps) {
        super(props);
    ...
    encodeVideo(buttonNumber: number) {
    ...
    getVideoFile(buttonNumber: number): string {
        return `${RNFS.CachesDirectoryPath}/video${buttonNumber}.mp4`;
    }

    cancel(buttonNumber: number) {
        let sessionId = 0;
