 ...
 */
function buildFFKitMemProtocolCommand(inputUrl: string, outputUrl: string, fontPath: string, text: string): string {
...
function humanReadableByteCount(bytes: number): string {
    if (bytes < 1024) {
        return `${bytes} B`;
    }
...
type FFKitProtocolsTabProps = TabProps<'FFKIT PROTOCOLS'>;

interface FFKitProtocolsTabState {
...
export default class FFKitProtocolsTab extends React.Component<FFKitProtocolsTabProps, FFKitProtocolsTabState> {
    ...
    constructor(props: FFKitProtocolsTabProps) {
        super(props);
    ...
    changedProtocol = (selectedProtocol: string) => {
        this.clearOutput();
        this.setState({
