...
type HttpsTabProps = TabProps<'HTTPS'>;

interface HttpsTabState {
    urlText: string;
    outputText: string;
}

export default class HttpsTab extends React.Component<HttpsTabProps, HttpsTabState> {

    private scrollViewReference: ScrollView | null = null;

    constructor(props: HttpsTabProps) {
        super(props);
    ...
    runGetMediaInformation = (buttonNumber: number) => {
        ...
        ffprint(`Testing HTTPS with custom ca bundle for button ${buttonNumber} using url ${testUrl}.`);
        ...
        let caCertificateBundlePath = VideoUtil.assetPath(VideoUtil.CA_CERTIFICATE_BUNDLE_ASSET);

        // GET MEDIA INFORMATION USING A CUSTOM COMMAND WITH A CA CERTIFICATE BUNDLE
        // PROVIDING A CA CERTIFICATE BUNDLE IS REQUIRED BY OPENSSL ON ANDROID/IOS UNLESS "-tls_verify 0" IS PROVIDED FOR FFMPEG 9+
        FFprobeKit.getMediaInformationFromCommandArgumentsAsync([
                ...
                "-i",
                testUrl
            ], async (ffprobeSession) => {
                // THE CALLBACK IS INVOKED WITH A MediaInformationSession, WHICH THE PACKAGE
                // DECLARATIONS TYPE AS THE FFprobeSession IT EXTENDS.
                const session = ffprobeSession as unknown as MediaInformationSession;
                const information: MediaInformation | undefined = await session.getMediaInformation();
                ...
                if (information === undefined) {
                    ...
                    let streams: StreamInformation[] | undefined = information.getStreams();
                    ...
                    let chapters: Chapter[] | undefined = information.getChapters();
