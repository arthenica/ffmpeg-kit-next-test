...
function assertNotNull(condition: unknown): void {
    if (condition == null) {
        throw `Assertion failed: ${condition} is null`;
    }
}

function assertIsArray(variable: unknown): void {
    if (!Array.isArray(variable)) {
        throw "Assertion failed";
    }
}

function assertEquals(expected: unknown, real: unknown): void {
    if (expected !== real) {
        throw `Assertion failed: ${real} != ${expected}`;
    }
}

function isNotSupportedException(error: any): boolean {
    const code = error?.code ?? "";
    const message = error?.message ?? error?.toString?.() ?? "";
    return code.toLowerCase().includes("not supported") || message.toLowerCase().includes("not supported");
}

function testParseSimpleCommand(): void {
...
function testParseSingleQuotesInCommand(): void {
...
function testParseDoubleQuotesInCommand(): void {
...
function testParseDoubleQuotesAndEscapesInCommand(): void {
...
export default class Test {
    ...
    static async testCommonApiMethods(): Promise<void> {
        ...
        ffprint(`Old log level: ${Level.levelToString(FFmpegKitConfig.getLogLevel() as unknown as number)}`);
        await FFmpegKitConfig.setLogLevel(Level.AV_LOG_INFO as unknown as Level);
        ffprint(`New log level: ${Level.levelToString(FFmpegKitConfig.getLogLevel() as unknown as number)}`)
    ...
    static testParseArguments(): void {
    ...
    static async getSupportedCameraIdsTest(): Promise<void> {
    ...
    static async setSessionHistorySizeTest(): Promise<void> {
