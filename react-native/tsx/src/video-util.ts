...
export default class VideoUtil {
    static get ASSET_1(): string {
        return "tree.jpg";
    }

    static get ASSET_2(): string {
        return "lake.jpg";
    }

    static get ASSET_3(): string {
        return "sunset.jpg";
    }

    static get SUBTITLE_ASSET(): string {
        return "subtitle.srt";
    }

    static get FONT_ASSET_1(): string {
        return "doppioone_regular.ttf";
    }

    static get FONT_ASSET_2(): string {
        return "notosansarabic_regular.ttf";
    }

    static get FONT_ASSET_3(): string {
        return "notosanssc_regular.ttf";
    }

    static get CA_CERTIFICATE_BUNDLE_ASSET(): string {
        return "cacert_2026_08_13.pem";
    }

    static async prepareAssets(): Promise<void> {
    ...
    static async assetToFile(assetName: string): Promise<string> {
    ...
    static assetPath(assetName: string): string {
        if (Platform.OS === 'ios') {
    ...
    static androidAssetPath(assetName: string): string {
        return `${RNFS.CachesDirectoryPath}/${assetName}`;
    }

    static iosAssetPath(assetName: string): string {
        return `${RNFS.MainBundlePath}/${assetName}`;
    }

    static generateEncodeVideoScript(image1Path: string, image2Path: string, image3Path: string, videoFilePath: string, videoCodec: string, customOptions: string): string {
    ...
    static generateEncodeVideoScriptWithCustomPixelFormat(image1Path: string, image2Path: string, image3Path: string, videoFilePath: string, videoCodec: string, pixelFormat: string, customOptions: string): string {
    ...
    static generateShakingVideoScript(image1Path: string, image2Path: string, image3Path: string, videoFilePath: string, videoCodec: string = 'mpeg4'): string {
    ...
    static generateCreateVideoWithPipesScript(image1Pipe: string, image2Pipe: string, image3Pipe: string, videoFilePath: string, videoCodec: string = 'mpeg4'): string {
    ...
    static async packageVideoCodec(): Promise<string> {
        const packageList = await Packages.getExternalLibraries();
        return packageList.includes('x264') ? 'libx264' : 'mpeg4';
    }

    static generateZscaleVideoScript(inputVideoFilePath: string, outputVideoFilePath: string): string {
        return "-y -i " +
            inputVideoFilePath +
